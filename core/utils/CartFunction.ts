 ...
 */
export const addFirstProduct = async (product) => {
    ...
    await SaveKeyPair('woo-next-cart', JSON.stringify(newCart));
 ...
 */
export const updateCart = async (existingCart, product, qtyToBeAdded, newQty = false) => {
...
export const removeItemFromCart = async (productId) => {
    ...
    let existingCart = GetValueForKey('woo-next-cart');
    existingCart = JSON.parse(existingCart);

    // If there is only one item in the cart, delete the cart.
    if (1 === existingCart.products.length) {

        await DeleteKeyPair('woo-next-cart');
        return null;
